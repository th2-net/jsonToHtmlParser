...
export default class SplitView extends React.Component<Props, State> {
	...
	private onResize(elements: ResizeObserverEntry[]) {
		if (elements[0]) {
			...
			const isVerticalNext = nextRootWidth < 600;
	...
	render() {
		...
		return (
			<div className={`${rootClassName} ${className ?? ''}`} ref={this.root} style={rootStyle}>
				{isDragging ? (
					<div
						className={createStyleSelector('', isVertical ? 'vertical' : null, 'splitter-preview')}
						style={previewStyle}>
						<div
							className={createStyleSelector(
								'',
								isVertical ? 'vertical' : null,
								'splitter-preview-left',
							)}
						/>
						<div
							className={createStyleSelector(
